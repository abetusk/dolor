function creatureBoneMinion() {
  this.x = 0;
  this.y = 0;
  this.d = "down";

  this.name = "bone_minion";
  this.dead_name = "dead_bone_minion";

  this.mask_dead_name = "mask_dead_bone_minion";
  this.mask_name = "mask_bone_minion";
  ...
  this.frame_lookup = {"down":0, "right":1, "left":2, "up":3, "none":0};
...
creatureBoneMinion.prototype.init = function(x,y, d) {
...
creatureBoneMinion.prototype.update_hit_bbox = function(bbox,x,y) {
  bbox[0][0] = x + 2;
  bbox[0][1] = y + 0;
...
creatureBoneMinion.prototype.update_bbox = function(bbox,x,y) {
  bbox[0][0] = x;
  bbox[0][1] = y;
...
creatureBoneMinion.prototype.hit = function(damage) {
  ...
  if (this.hp<=0) {
    ...
    if (this.bomb) { this.bomb.hide = false; }
...
creatureBoneMinion.prototype.world_collision = function(world) {
  var d = this.d;
  var new_d = d;
...
creatureBoneMinion.prototype.update_intent = function(d) {
  this.intent.d = d;
...
creatureBoneMinion.prototype.realize_intent = function() {
...
creatureBoneMinion.prototype.choose_direction = function(dx,dy) {
  var d = "none";
...
creatureBoneMinion.prototype.update = function(world) {
  var intent_d = this.d;
  ...
  if (this.state == "alive") {
    ...
    var count=0;
    for (var ind in world.element) {
      var ele = world.element[ind];

      if (ele.type == "bomb") {
        if (ele.being_held) { continue; }
        ...
        if ((Math.abs(dx)<16) && (Math.abs(dy)<16)) {
          ...
          var intent_d = this.choose_direction(dx,dy);
          this.update_intent(intent_d);
          this.skip_intent=true;

          //console.log(dx,dy, this.intent.d);
  ...
  if (this.state == "armed") {
    ...
    if (this.bomb.ttl<=0) {
      this.bomb.hide = false;
      this.bomb = null;
      this.state = "alive";

      if (this.d == "none") {
        this.update_intent("down");
      }
...
creatureBoneMinion.prototype.draw = function() {
